...
class GBVBDataset(Dataset):
    def __init__(self,
        ...
        self.ratio_vb2gb = len(self.VB_tickers) / len(self.GB_tickers)  # i.e., VB / GB
    ...
    def __getitem__(self, idx):
        ...
        MR_ticker = self.merged_tickers[mr_idx]
...
if __name__ == '__main__':
    ...
    dataset = GBVBDataset(kind='train')
    ...
    # test validity of samples
    encoder = nn.Sequential(nn.Conv1d(1, 32, kernel_size=7, stride=2),
                            nn.BatchNorm1d(32),
                            nn.GELU(),
                            nn.Conv1d(32, 512, kernel_size=3, stride=2),
                            nn.BatchNorm1d(512),
                            nn.GELU(),
    ...
    for batch in data_loader:
        ...
        # print(out)
        # print(out.shape)
        print(np.isnan(out.detach().numpy()).sum())
